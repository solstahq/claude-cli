...
- Shell state does not persist between Bash tool calls. Capture the token fresh in each command that uses the API that needs it: `TOKEN=$(scripts/get-token.sh <org> <stage>); curl -H "Authorization: Bearer $TOKEN" ...`
...
|------|-----|
| Login | CLI |
| List products/envs/repos/releases | API |
| Managing local installs | CLI |
